...
function filter_and_order_articles(_articles, c_id, q, s, o) {
  if (c_id != null) {_articles = _articles.filter(({ category_id }) => category_id.includes(parseInt(c_id)));}
  if (s != null) {_articles = _articles.filter(({ status }) => status == s);}
  if (q != null) {_articles = _articles.filter(({ description }) => description.toLowerCase().includes(q.toLowerCase()) == true);}
  if (o != null) {
    if (o == 'date') {_articles = _articles.sort((a, b) => new Date(b.date) - new Date(a.date));} 
    else if (o == 'score') {_articles = _articles.sort((a, b) => b.score - a.score);}
  }
  return _articles
}
...
function update_search_filters(o,s){
  ...
  if(s != null && s==0){s_options='<option selected value="0">Agotado</option><option value="1">Disponible</option>';}
  ...
  if(o != null && o=='score'){s_options='<option value="date">Fecha de publicación</option><option selected value="score">Relevancia</option>';}
  document.getElementById("o").innerHTML += o_options;
}

function get_article_by_id(id){
  for (var i in _articles) {
    if (parseInt(_articles[i]['id'])==parseInt(id)){return _articles[i];}
  }
  return null
}
...
function get_category_name_by_id(o_id){
  for (var i in _categories) {
    if (parseInt(_categories[i]['id'])==parseInt(o_id)){return _categories[i]['name'];}
  }
  return -1
}
...
function fetch_contact(c_message){
  ...
  r_contact=fetch("https://5mus0rdlc0.execute-api.eu-west-3.amazonaws.com/default/criptochollos_contact?"+c_message, requestOptions).then(response => response.text()).then(result => console.log(result)).catch(error => console.log('error', error));
}